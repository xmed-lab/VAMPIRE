...
from dataset import octainhouse_2d_dataset
...
test_weight_path = 'sam_weights/SAM_OCTA/{}/3M_LargeVessel_Global.pth'.format(args.model_type)
...
with torch.no_grad():
    for images, prompt_points, prompt_type, sample_ids in tqdm(val_loader):
        images, prompt_type = map(to_cuda, (images, prompt_type))
        ...
        image, pred = map(lambda x:x[0][0].cpu().detach().numpy(), (images, preds))

        if not os.path.exists(save_dir): os.makedirs(save_dir)
        save_sample_func = lambda x, y: y.save("/".join([save_dir, "{}.png".format(sample_id)]), format='PNG', compress_leve=0)
