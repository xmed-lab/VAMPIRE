...
def load_pretrained_weights_safe(model, pretrained_weights, checkpoint_key, model_name, patch_size):
    def filter_state_dict(state_dict, model_state):
        # 筛选出那些与当前模型参数形状匹配的权重
        for k in list(state_dict.keys()):
            if k in model_state:
                if state_dict[k].shape != model_state[k].shape:
                    print(f"Skipping loading parameter {k} due to shape mismatch: {state_dict[k].shape} vs {model_state[k].shape}")
                    del state_dict[k]
        return state_dict
    ...
    if os.path.isfile(pretrained_weights):
        ...
        # 去除 `module.` 前缀
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        # 去除由 multicrop wrapper 产生的 `backbone.` 前缀
        state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
        # state_dict = {k.replace("base_encoder.", ""): v for k, v in state_dict.items()}
        ...
        # 检查其他参数的维度是否匹配
        state_dict = filter_state_dict(state_dict, model.state_dict())
