...
class InfoProjector(nn.Module):
    def __init__(self, output_dim, hidden_dim=64):
        super().__init__()
        self.age_mlp = MLPFeatureProjector(1, output_dim, hidden_dim)
        self.gender_mlp = MLPFeatureProjector(2, output_dim, hidden_dim)

    def forward(self, a, g):
        age_features = self.age_mlp(a.unsqueeze(1))
        gender_features = self.gender_mlp(g)

        return age_features, gender_features
    
class PromptLearner(nn.Module):
    def __init__(self, n_ctx=16, n_prompt_tokens=8, initialization=None):
        super().__init__()
        
        # 创建可学习的prompt向量
        self.prompt_embeddings = nn.Parameter(torch.randn(n_prompt_tokens, n_ctx))
        
        # 如果有初始化值
        if initialization is not None:
            self.prompt_embeddings.data = initialization

class ModelWithPrompt(nn.Module):
    def __init__(self, n_prompt_tokens=8):
        super().__init__()
        ...
        self.prompt_learner = PromptLearner(n_ctx=embedding_dim, n_prompt_tokens=n_prompt_tokens)
        
    def forward(self, text, device):
        tokens = self.tokenizer(text,padding=True,truncation=True,max_length=512,return_tensors="pt").to(device)
        
        x = self.text_encoder.embeddings.word_embeddings(tokens.input_ids)
        
        prompts = self.prompt_learner.prompt_embeddings
        prompts = prompts.unsqueeze(0).expand(x.shape[0], -1, -1)
        x = torch.cat([prompts, x], dim=1)
        
        attention_mask = tokens.attention_mask
        prompt_attention = torch.ones((attention_mask.shape[0], self.prompt_learner.prompt_embeddings.shape[0]), 
                                    device=attention_mask.device)
        extended_attention_mask = torch.cat([prompt_attention, attention_mask], dim=1)
        
        outputs = self.text_encoder(inputs_embeds=x, attention_mask=extended_attention_mask)
